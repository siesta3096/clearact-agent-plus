...
- 任意已执行阶段都可附带反馈重新开始：此前阶段的消息和结果直接复用，后续消息会分叉重建，本地文件写入会按快照逆序恢复，无法撤销的外部副作用会明确提示。
...
- **MCP 客户端接入**：支持本地 `stdio` 与远程 Streamable HTTP 服务；设置页可直接添加、启停、编辑和测试连接，也保留 JSON 导入；多个启用服务会并行发现，避免启动耗时叠加。
